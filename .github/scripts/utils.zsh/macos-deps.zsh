...
if [[ ! -d ./.deps/boost ]]; then
  ...
  mkdir -p .deps/boost
  pushd .deps/boost
  curl https://archives.boost.io/release/1.91.0/source/boost_1_91_0.tar.gz > boost.tar.gz
...
if [[ ! -d ./.deps/openssl ]]; then
  ...
  pushd .deps/openssl
  curl -L https://github.com/openssl/openssl/releases/download/openssl-4.0.1/openssl-4.0.1.tar.gz > openssl.tar.gz
  ...
  make -j16 -s
  chmod -R +w .  # For some reason the built files aren't writable by default
  popd
else
